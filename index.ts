...
const SUPPORTED_VARIANTS = ['chat', 'call', 'callwithchat'];
...
const spawnChildServer = async (version: string, variant: string): Promise<void> => {
  ...
  const scriptPath = path.join(__dirname, 'versions', version, variant, `server.cjs`);
...
const server = http.createServer(async (req, res) => {
  ...
  if (!variant || !(SUPPORTED_VARIANTS.includes(variant))) {
